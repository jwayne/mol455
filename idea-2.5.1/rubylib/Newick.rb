...
class NewickTree
  ...
  # quotes names to protect odd characters
  def quoteNames
    @root.descendants.each {|node|
      #node.name = "'" + node.name + "'"
      node.name.gsub!("_", " ")
  ...
  def draw(pdfFile, boot="width", giLink = true, labelName = false,
	   highlights = [])
    require 'fpdf'
    pdf=FPDF.new('P', "cm")
	...
	highlights.each{|highlight|
	  hl = true if (entry.index(highlight))
	}
        if (giLink && (entry =~ /gi[\_]*([0-9]*)/ || entry =~ /(^[A-Z|0-9]*)\|/))
          species = entry if species.nil?
	  pdf.SetTextColor(255,0, 0) if hl # red
          pdf.Write(0, species, ncbiLink + protLink + $1)
